...
func Test_app_renderWithBadTemplate(t *testing.T) {
	...
	if err := app.render(rr, req, "bad.page.gohtml", &TemplateData{}); err == nil {
		t.Error("expected error from bad template, did not get one")
	}
}

func getCtx(req *http.Request) context.Context {
	ctx := context.WithValue(req.Context(), ctxUserKey, "unknown")
	return ctx
}
...
func Test_app_home(t *testing.T) {
	...
	for _, e := range tests {
		...
		if rr.Code != http.StatusOK {
			t.Errorf("Test_app_home: wrong status code; expected %d, got %d", http.StatusOK, rr.Code)
		}
